...
public List<Product> filter(int categoryId, String propertyName, String order) {
    ...
    if (!validOrders.contains(order.toUpperCase())) {
        order = "ASC"; // Default order ascending if invalid
    }

    String sql = "SELECT * FROM `PRODUCTS` ORDER BY " + propertyName + " " + order;
    try {
        PreparedStatement stmt = con.prepareStatement(sql);
        ResultSet rs = stmt.executeQuery();
        while (rs.next()) {
            ...
            proList.add(new Product(id, name, description, thumbnail, price, quantity, view, 0, createdAt));
        }
    } catch (SQLException e) {
        e.printStackTrace();
    }
